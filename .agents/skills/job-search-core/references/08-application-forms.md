...
**Every claim in a form field must already be defensible from the same private evidence sources used for the CV and cover letter**: `profile/candidate-profile.md`, attached user-provided evidence, and any private baseline CV the user explicitly designates. The public `cv/main_example.tex` is synthetic layout material, never candidate evidence. A form field is not a place to introduce claims, inflate scope, or fill space; it selects and arranges facts already known to be true.
...
- [ ] Every factual claim traces to private candidate evidence or user-provided source material; the public example documents are never evidence
