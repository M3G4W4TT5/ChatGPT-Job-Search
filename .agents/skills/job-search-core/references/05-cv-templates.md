...
Candidate-specific statements, section preferences, and custom template choices belong in `profile/`; this shared reference remains immutable.
...
**Layout reference:** `cv/main_example.tex` is synthetic and may be used only for structure, typography, and compilation behavior. Candidate facts must come from private, user-approved evidence.
...
Statements labeled *[Used for: <company>_<role>]* were extracted from archived application drafts by `the setup skill` Path A. They are **phrasing references, never fact sources**: when drafting from one, every factual claim still comes from `profile/candidate-profile.md` - a past tailored draft does not vouch for its own accuracy.
...
3. Render every PDF page to an image and visually inspect both pages
...
```powershell
Push-Location cv
pdftotext -layout main_<company>_<role>.pdf main_<company>_<role>.txt
Pop-Location
```

`pdftotext` comes from [Poppler](https://poppler.freedesktop.org/), not the TeX distribution. It is required for a passed local ATS gate. If it is unavailable, report the gate as not run rather than treating visual inspection as equivalent.
