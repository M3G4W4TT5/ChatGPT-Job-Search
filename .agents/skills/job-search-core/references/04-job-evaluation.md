...
Candidate-specific eligibility, language, goals, and weighting overrides belong in `profile/evaluation-preferences.md`; this shared reference remains immutable.
...
If the candidate's permit also constrains *hours* or *start date* (a student visa with a term-time cap, a permit that begins on graduation), record that as a second gate in `profile/evaluation-preferences.md`, with the specific dates. Do not merge it with the eligibility question above because they fail for different reasons and need different answers.

A role that fails this gate is not scored and not drafted. Everything below applies only to roles that pass it.

## Language Gate — run before scoring

Run this gate before the weighted dimensions: read the posting, classify against the candidate's declared profile data, and treat a hard mismatch as FAIL before scoring.

Read the posting's language requirements as stated for **the role itself** — not the language the ad happens to be written in. A posting written in a language you don't work in, for a role that only needs languages you do work in on the job, passes fine; only an explicit job-condition requirement ("fluent X required," "must communicate with the Y team in Z") triggers this check. For each language the posting requires as a job condition, compare it against your Languages table in `profile/candidate-profile.md`:
...
Classify strong, moderate, and missing skills from the private candidate profile for the current job. Do not keep candidate-specific skill lists in this shared reference.
...
Classify direct, adjacent, and limited experience from cited private evidence for the current job. Missing evidence remains missing.
...
Read career goals, energizing and draining tasks, autonomy preferences, flexibility constraints, and growth priorities from the private evaluation preferences. If they are absent, this dimension is unavailable; do not infer a neutral score.
...
### Recommendation
[1-2 sentences: apply, skip, or use the apply skill with stated caveats]
...
Calculate the overall score as the nearest whole number from the four weighted dimensions. If any weighted dimension lacks enough private evidence to score honestly, do not assign zero, a neutral score, or renormalize the remaining weights. Return `needs_profile`, list the missing evidence, and withhold the overall score until the profile is complete.
